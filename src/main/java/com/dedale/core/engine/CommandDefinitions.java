...
public class CommandDefinitions implements Iterable<CommandDefinition> {
    ...
    public CommandDefinitions withModule(String keyword, CommandModule module) {
    ...
    public CommandDefinitions flatAdd(CommandDefinitions commands) {
    ...
    public CommandDefinitions withCommand(String keyword, Supplier<Expression> supplier) {
        return andParse(keyword).with().provider(supplier).build();
    }
